...
class App extends Component {
  ...
  startQuiz() {
    this.setState({ started: true, score: null, seconds: 60, count: 0 });
  ...
  renderDisplayText() {
    return this.state.text.map((word, index) => {
      return (
        <span
          key={index}
          className={index === this.state.count ? `highlight` : ""}
        >{`${word} `}</span>
      );
    });
  }
      ...
      <React.Fragment>
        <nav className="navbar navbar-dark bg-dark justify-content-between">
          ...
          <div className="form-inline">
            <h6 className="btn btn-danger">{`Time Left: ${
              this.state.seconds
            } Secs`}</h6>
          </div>
        </nav>
        <div className="container">
          ...
          {this.state.started ? (
            <React.Fragment>
              ...
              <div className="row">
                <div className="col-md-6 offset-md-3">
                  <a href="#"  className="input-box btn btn-primary">
                    End Test
                  </a>
                </div>
              </div>
            ...
            <div className="row">
              <div className="col-md-6 offset-md-3">
                <a
                  ...
                  className="input-box btn btn-primary"
                >
                  Start Test
                </a>
              </div>
